...
- Regenerate the generator-owned terminology config from the local real corpus archive: `python3 scripts/update_terminology_from_corpus.py`
- Verify that the committed terminology config matches the current local archive: `python3 scripts/update_terminology_from_corpus.py --check`
...
- Run the scan-vs-FTS retrieval usefulness comparison against the committed default runtime: `python3 scripts/run_retrieval_backend_comparison.py --baseline-config configs/runtime.scan.yaml --candidate-config configs/runtime.yaml`
- Re-run fixture eval against an explicit runtime config: `python3 scripts/run_eval.py --all --runtime-config configs/runtime.sqlite_fts.yaml`
- Re-run real-corpus eval against an explicit runtime config: `python3 scripts/run_eval.py --all --catalog artifacts/real_corpus/curated_catalog.json --runtime-config configs/runtime.sqlite_fts.yaml`
...
- Generate the validated current-state report against an explicit runtime config: `python3 scripts/report_validated_current_state.py --runtime-config configs/runtime.sqlite_fts.yaml`
...
- `configs/terminology.yaml` is generator-owned output from `scripts/update_terminology_from_corpus.py`; regenerate it after archive or curated-catalog changes instead of editing it by hand.
...
- `configs/runtime.yaml` is the fixed default runtime and now resolves to `sqlite_fts` without a normal backend toggle; `configs/runtime.scan.yaml` remains only as a reproducible comparison baseline.
- `eval_run_manifest.json`, `pack_run_manifest.json`, `spawned_validator_gate_manifest.json`, and `validated_current_state_report.json` now record the runtime config path, a config digest, and the active local retrieval backend so release review can bind to a specific retrieval configuration instead of only the facade contract id.
...
- `relation_hints.json` is a post-ledger, per-run supplemental artifact derived only from `approved_ledger.json`; when a relation hint is rendered into `final_answer.txt`, it still flows through the normal bullet-backed `pinpoint_evidence.json` and `answer_alignment.json` chain rather than a separate prose path.
