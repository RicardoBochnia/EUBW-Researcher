...
The validated current-state report lives outside per-run bundles under `artifacts/current_state/`; it treats the real-corpus eval manifest as the authoritative binding gate surface and only records real-question-pack evidence as supplemental context when a pack manifest is provided.
